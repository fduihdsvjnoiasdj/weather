...
const PRESETS = [
  {
    ...
    logic: 'AND', consecutiveHours: null, checkTime: '08:00'
  },
  {
    name: 'Mráz',
  ...
  },
  {
    ...
    logic: 'AND', consecutiveHours: 4, checkTime: '08:00'
  }
];
...
function generateRuleSummary(rule) {
  ...
  text += ` · denně v ${rule.checkTime || '08:00'}`;
  return text;
}

/* ---- Rule Editor ---- */

let editorState = { conditions: [], logic: 'AND', consecutiveHours: null, checkTime: '08:00' };
...
function openRuleEditor(loc, existingRule) {
  ...
  if (existingRule) {
    editorState = {
      ...
      checkTime: existingRule.checkTime || '08:00'
    };
  } else {
    editorState = {
      id: null,
...
function renderRuleEditor(container, loc, lk) {
  let html = `
    <div class="rule-editor">
      ...
      <div class="rule-field">
        <div class="rule-field-label">Čas denního upozornění</div>
        <input type="time" id="check-time-input" value="${editorState.checkTime}">
      </div>
  ...
  document.getElementById('editor-save').addEventListener('click', () => {
    ...
    editorState.checkTime = document.getElementById('check-time-input').value || '08:00';
    ...
    const rule = {
      ...
      checkTime: editorState.checkTime
    };
